...
y1, y2, y3, y4, y5 = b, HEIGHT // 4 - \
    d, HEIGHT // 4, HEIGHT // 2 - 1.5 * b, HEIGHT - 4 * b
...
for i in range(10):
    ...
    for j in range(WIDTH // step + 2):
        ...
        if i == 9:
            create_segment((peg_x, peg_y + 50), (peg_x, HEIGHT),
                           segment_thickness, space, 'darkslategray')
        peg_x += step
    peg_y += 0.5 * step
...
# balls
balls = [([randrange(256) for i in range(3)], create_ball(space))
         for j in range(600)]
